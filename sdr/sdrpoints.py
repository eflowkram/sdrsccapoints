...
def class_header_text(event_count):
    ...
    for i in range(1,event_count+1):
      e+=f"{'Event '}{i : <5}"
    h=f"\n{'Place' : <10}{'Driver' : <25}{'Car' : <8}{'Class' : <10}"+e+f"{' Points' : <11}{'Cones' : <8}{'DNF' : <5}"
    return h 
...
def class_standings(driver_id,car_class):
    ...
    cs=[results[0][0],results[0][1],car_class,results[0][2],results[0][3],results[0][4]]
    return cs,ep
    ...
    if args.print_points:
        ...
        for c in car_class:
          ...
          if args.output=='text':
            ...
            h=class_header_csv(event_count)
            print(h)
          for l in results:
